...
trait NodeScala {
  ...
  private def respond(
      ...
      response: Response): Unit = future { //TODO: check if future?
    while(token.nonCancelled && response.hasNext) {
      val responseItem = response.next
      exchange.write(responseItem)
    }
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val mysubscription = CancellationTokenSource()
    val ret = Subscription(listenerSubscription, mysubscription)
    async {
      while(mysubscription.cancellationToken.nonCancelled) {
        val (request, exchange) = await (listener.nextRequest)
        val response = handler(request)
        respond(exchange, mysubscription.cancellationToken, response)
      }
    }
    ret
  }
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext { handler =>
        p.success(Tuple2(handler.request, handler))
        removeContext()
      }
      //p.future.onComplete(_ => removeContext())
      p.future
    }
  }
